...
// Port of the Mac HomeView; phone layout: stat tiles 2×2, cards stack.
...
struct HomeView: View {
        ...
        Page(kicker: monoDate(), title: "\(greeting()), \(firstName)") {
            GlassEffectContainer(spacing: 14) {
                VStack(alignment: .leading, spacing: 14) {
                    statsRow
                    runningNow
                    // ViewThatFits (not a size-class branch): iPad portrait is
                    // regular-width but ~500pt, where the fixed 300pt weekCard
                    // would crush the feed to a sliver. Stack when narrow.
    ...
    // MARK: stats — 2×2 on phone, adaptive row on iPad
